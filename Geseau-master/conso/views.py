...
def index(request):
    if request.user.is_authenticated:
        ...
        # Récupérer les sections associées à l'utilisateur
        sections = Section.objects.filter(entreprise=user_entreprise)
        
        if not sections.exists():
            # Gérer le cas où aucune section n'est associée à l'utilisateur
            return render(request, 'conso/index.html', context={'message': 'Aucune section trouvée pour cet utilisateur.'})
        ...
        daily_consommation = Consommation.objects.filter(
            dispositif__section__in=sections,
            created_at__range=(start_of_day, end_of_day)
        ).aggregate(Sum('quantite'))['quantite__sum'] or 0

        weekly_consommation = Consommation.objects.filter(
            dispositif__section__in=sections,
            created_at__date__range=[start_of_week, end_of_week]
        ).aggregate(Sum('quantite'))['quantite__sum'] or 0

        monthly_consommation = Consommation.objects.filter(
            dispositif__section__in=sections,
            created_at__date__range=(month_start, month_end)
        ).aggregate(Sum('quantite'))['quantite__sum'] or 0

        # Consommations par section
        daily_consommation_section = []
        weekly_consommation_section = []
        monthly_consommation_section = []

        for section in sections:
            daily = Consommation.objects.filter(
                dispositif__section=section,
                created_at__date__range=(start_of_day, end_of_day)
            ).aggregate(Sum('quantite'))['quantite__sum'] or 0
            weekly = Consommation.objects.filter(
                dispositif__section=section,
                created_at__date__range=(start_of_week, end_of_week)
            ).aggregate(Sum('quantite'))['quantite__sum'] or 0
            monthly = Consommation.objects.filter(
                dispositif__section=section,
                created_at__date__range=(month_start, month_end)
            ).aggregate(Sum('quantite'))['quantite__sum'] or 0

            daily_consommation_section.append(daily)
            weekly_consommation_section.append(weekly)
            monthly_consommation_section.append(monthly)
        ...
        ).values('created_at__date').annotate(quantite_sum=Sum('quantite'))
        ...
        consommations = Consommation.objects.filter(dispositif__section__in=sections)
        ...
        context = {
            ...
            'daily': daily,
            "daily_consommation_section": [round(x, 3) for x in daily_consommation_section],
            "weekly_consommation_section": [round(x, 3) for x in weekly_consommation_section],
            "monthly_consommation_section": [round(x, 3) for x in monthly_consommation_section],
        }
